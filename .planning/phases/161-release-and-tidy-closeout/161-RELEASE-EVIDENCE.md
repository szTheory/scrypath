...
| Final PR head SHA | Final evidence commit; exact SHA and hosted run are in `/private/tmp/scrypath-161-final-closeout.json` |
...
- Base: `main`; head: `gsd/v1.37-code-quality-ratchet`; PR_CREATED_SHA: `3053efcf06bf574d599e3969385dfd904d31ca53`. The final evidence commit becomes the current PR head; its exact SHA and checks are in `/private/tmp/scrypath-161-final-closeout.json`.
- Authorization to create was explicitly received. No merge authorization was given.
- Checks passed on the PR creation SHA. The final evidence commit is rechecked on the exact SHA after this record is committed. No maintainer review, approval, or review comments are recorded. Do not merge or claim the release shipped until the actual review/merge and publication gates pass.
...
Run `node scripts/ci_monitor.cjs closeout --push --branch "$(git branch --show-current)" --sha "$(git rev-parse HEAD)"` after the final evidence/summary commit. Retain the exact SHA, run URL, five required jobs, coverage/attestation IDs, and digests in `/private/tmp/scrypath-161-final-closeout.json` and the executor's final response. No tracked edits follow that run.
