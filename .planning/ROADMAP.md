...
- [ ] **Phase 40 — `:all` expansion** — not started — **FED-02** (explicit `:all` or equivalent token, documented resolution rule, cardinality rails, timeouts, explicit `{:error, _}` paths; tests for happy + rejection paths).
