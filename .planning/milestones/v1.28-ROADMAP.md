# Milestone Archive: v1.28 — Realistic Demo App & Admin UI Proof

**Archived:** 2026-05-31
**Outcome:** Shipped + archived in-repo
**Phases:** 102-105
**Total plans:** 15
**Audit:** Tech debt only — 15/15 requirements, 4/4 phases, 4/4 flows; see `v1.28-MILESTONE-AUDIT.md`.
...
- [PHASE97-SCOPE-GUARD] Phase 97/98/99 reject runtime breadth expansion unless reopen policy conditions are met. See `.planning/milestones/v1.27-phases/97-canonical-contract-freeze-and-scope-guard/97-SCOPE-GUARD.md`.

## Milestone Summary

**Key decisions:**

- `scrypath_ops` is a mountable router engine instead of a standalone-only Phoenix application.
- The demo proof uses a realistic multi-tenant e-commerce app rather than a trivial blog-style domain.
- Storefront search remains application-owned LiveView code; Scrypath supplies Ecto-native indexing/search behavior without reusable UI widgets.
- Playwright owns browser assertions while Mix and CI own Phoenix/service lifecycle.
- `phase105-e2e` is advisory while stability history accumulates.

**Issues deferred:**

- Category-filtered `/dev/e2e/search-visible` readiness probes should merge filters instead of replacing the tenant filter.
- Promotion of `phase105-e2e` from advisory to required remains a release-policy decision after stability evidence.
- Some validation frontmatter remains partially Nyquist-compliant for older v1.28 phases.

**Technical debt incurred:**

- The example harness contains dev/test-only operator probes that must remain outside public Scrypath APIs.
- `scrypath_ops` startup still has duplicated branch shape around `validate_opsui_auth_on_start` to preserve existing boot-contract tests.
