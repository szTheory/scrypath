# Milestone archive: v1.9 — Per-query relevance & tuning pipeline

**Archived:** 2026-04-20  
**Hex:** `scrypath` **0.3.3** (per-query spec + runtime shipped in-repo at archive; no mandated Hex bump in this planning milestone)  
**Phases:** 42–43 (**2** phases, **5** plans)  
**Audit:** Post-close ledger **`milestones/v1.9-MILESTONE-AUDIT.md`**. **`audit-open`** items **acknowledged** at close — see **`STATE.md`** § Deferred Items.  
