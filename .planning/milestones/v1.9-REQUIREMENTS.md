# Requirements archive: Scrypath v1.9

**Archived:** 2026-04-20  
**Milestone:** v1.9 — *Per-query relevance & tuning pipeline*  
**Outcomes:** **TUNE-PIPE-01..04** and **TUNE-PQ-01..03** satisfied per phase summaries, **`43-VERIFICATION.md`**, and **`mix verify.phase43`** in CI. **OPSUI-01** remains explicitly deferred (see archived roadmap backlog).
...
- [x] **TUNE-PQ-01**: Application code can request **per-query relevance or settings overrides** that behave exactly as the locked **`TUNE-PIPE-*`** spec describes for **`Scrypath.search/3`**, with documented rules for **`search_many/2`** when applicable.
- [x] **TUNE-PQ-02**: Automated tests and an agreed **`mix verify.phaseNN`** (or successor slice) cover **happy paths**, **rejection paths**, and **regressions** for the per-query path.
- [x] **TUNE-PQ-03**: Documentation plus **`docs_contract_test.exs`** (or an agreed equivalent) **anchors** the public contract so README / guides / ExDoc cannot drift silently.
...
| TUNE-PQ-01 | Phase 43 | Complete |
| TUNE-PQ-02 | Phase 43 | Complete |
| TUNE-PQ-03 | Phase 43 | Complete |
...
*Last updated: 2026-04-20 — **v1.9** complete (**Phase 42–43**); file archived to **`milestones/v1.9-REQUIREMENTS.md`** at milestone close.*
