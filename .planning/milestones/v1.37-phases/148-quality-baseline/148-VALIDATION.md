...
|---|---|---|---|---|
| QUAL-01 | Ledger schema/source inspection — supported by prior committed evidence | Ranked evidence/disposition fields are present; no claim that later ledger dispositions follow automatically. | `.planning/reference/QUALITY-LEDGER.md`, receipt `92abcd9`; matrix QUAL-01. | covered — durable ledger contract is mapped. |
...
| TEST-05 | Exact-SHA hosted CI coverage producer and artifact — present-state verified | Scheduled/manual advisory wiring, successful producer, successful upload, and downloaded nonempty report are independently recorded. | Run `33014343041`, source/head/candidate `a35874178b79392caa0f3c1dcc010ea149e1e5bf`; receipt `159-CLOSURE-RECEIPT.md`. | covered — hosted TEST-05 closure is evidence-backed. |

## Compliance

`nyquist_compliant: true`: TEST-05 hosted proof is complete. TEST-01 remains the sole narrow historically-unprovable chronology waiver; TEST-03 retains its original evidence class without changing ownership.
