...
Plans:
- [ ] 90-01: Create the first-class `Scrypath.Oban.Worker` template specifically designed for durably enqueueing related-data fan-out updates.
- [ ] 90-02: Implement failure state collection in `Scrypath.sync_related/3` so callers receive `{:error, %{failed_records: [...]}}`.
- [ ] 90-03: Add integration tests simulating partial backend failures during multi-record fan-out updates.
