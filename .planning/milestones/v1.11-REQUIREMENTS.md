# Requirements archive: Scrypath v1.11 — Operator shell polish and JTBD verification

**Archived:** 2026-04-21  
**Milestone:** v1.11 — *Operator shell polish and JTBD verification*  
**Outcomes:** **OPSUX-01**..**OPSUX-07** satisfied per reconciled traceability and phase **48–50** summaries (IA contract, posture JTBD, **`/ops`** scaffold and themes, Phoenix shell tests, accessibility + CI slice).
...
- [x] **OPSUX-03**: **`/ops/posture`**, **`/ops/failed-sync`**, **`/ops/sync-drift`**, and **`/ops/search`** use **consistent page structure** (title, primary panel, secondary detail) so operators can **scan** lists, rollups, and warnings in **light and dark** themes.

### Theming and Phoenix ergonomics

- [x] **OPSUX-04**: **System, light, and dark** theme choices work end-to-end (including first paint / persistence), respect **operating-system preference** when set to system, and remain **readable** (contrast, borders, focus rings) on all **`/ops`** screens.
- [x] **OPSUX-05**: UI patterns follow **conventional Phoenix LiveView** expectations (layouts, flash, links, components) per **OPSUI-07** spirit — fix **inconsistencies** found during audit (for example stray default shell chrome, duplicate titles, unclear errors).

### Accessibility and verification

- [x] **OPSUX-06**: **Basic accessibility**: logical **heading** order, **`main`** / landmark usage where appropriate, and **labels** for interactive controls on operator-critical paths (triage tables, playground controls, federation inspector toggles as applicable).
- [x] **OPSUX-07**: **CI regression** coverage is extended for any new IA or nav contracts (and critical LiveView paths), so **OPSUI-10** discipline continues after polish — no decrease in operator wiring guarantees.
...
| OPSUX-03 | Phase 49 | Done |
| OPSUX-04 | Phase 49 | Done |
| OPSUX-05 | Phase 49 | Done |
| OPSUX-06 | Phase 50 | Done |
| OPSUX-07 | Phase 50 | Done |

**Coverage:** v1.11 requirements: **7** total · Mapped: **7** · Unmapped: **0** ✓

---
*Requirements defined: 2026-04-21 after `/gsd-new-milestone` v1.11 · reconciled to shipped evidence **2026-04-21** (phases **48–50**) before milestone archive*
