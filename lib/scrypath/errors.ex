defmodule Scrypath.Errors do
  ...
  def format_reason(reason) do
    case reason do
      ...
      other ->
        generic_reason(other)
    end
  end

  defp generic_reason({:transport_failed, _}) do
  ...
  defp generic_reason({:timeout, _}) do
  ...
  defp generic_reason({:task_failed, _}) do
  ...
  defp generic_reason({:cancelled, _}) do
  ...
  defp generic_reason({:invalid_task_payload, _}) do
  ...
  defp generic_reason(other), do: "Search or sync failed: #{inspect(other)}"
end
