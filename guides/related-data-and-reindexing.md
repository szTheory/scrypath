...
For ordinary schemas, declare fan-out with `use Scrypath, fan_outs:`. Scrypath generates
`__scrypath__(:fan_outs)` for this path, and `Scrypath.sync_related/3` consumes that
generated `__scrypath__(:fan_outs)` metadata at runtime.

Hand-written `__scrypath__/1` remains supported as a low-level escape hatch for
owner-only schemas that intentionally do not `use Scrypath`; it is not the normal
copy-paste path for searchable schemas.

The `document_id` reflection key is already generated by `use Scrypath`. Define it by hand
only if the owning schema does not use `use Scrypath` but still participates in fan-out.
